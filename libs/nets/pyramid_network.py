...
def build_heads(pyramid, ih, iw, num_classes, base_anchors, is_training=False, gt_boxes=None,num_joints =14):
  ...
  with slim.arg_scope(arg_scope):
    with tf.variable_scope('pyramid'):
        ...
        m = slim.conv2d_transpose(m, 256, 2, stride=2, padding ='VALID', activation_fn=tf.nn.relu)
        tf.add_to_collection('__TRANSPOSED__', m)
        m = slim.conv2d(m, num_joints, [1, 1], stride=1, padding='VALID', activation_fn=None)
  ...
  with slim.arg_scope(arg_scope):
      with tf.variable_scope('pyramid'):
        ...
        labels, mask_targets, mask_inside_weights = \
          mask_encoder(gt_masks, gt_boxes, ordered_rois, num_joints, 56, 56, scope='MaskEncoder')
        ...
        # apply softmax_cross_entropy which encourage single point to be detected
        with tf.device("/cpu:0"):
            mask_loss_tmp = []
            for joints_id in range(num_joints):
                mask_loss_tmp.append(
                    mask_lw * tf.nn.softmax_cross_entropy_with_logits(
                        labels=slim.flatten(mask_targets[:,:,:,joints_id]),
                        logits=slim.flatten(masks[:,:,:,joints_id])
                        )
                    )
        mask_loss = tf.reduce_mean(mask_loss_tmp)
...
def build(end_points, image_height, image_width, pyramid_map,
    ...
    # choose the max score mask to visualise

    # print ('final_prob:',outputs['final_boxes']['prob'])
    # print ('final_cls:',outputs['final_boxes']['cls'])
    # print ('final_box:',outputs['final_boxes']['box'])
    # print ('pred_clc:', outputs['mask']['cls'].shape)
    # print ('pred_score:', outputs['mask']['score'].shape)
    # print ('mask:', outputs['mask']['mask'])
    # print (outputs['mask']['mask'][:1].shape)
    # raise
