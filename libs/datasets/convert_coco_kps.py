    ...
    for shard_id in range(0,int(num_shard)):
        ...
        with tf.python_io.TFRecordWriter(record_filename) as tfrecord_writer:
            for id in range( num_per_shard*shard_id, min(num_per_shard*shard_id + num_per_shard, num_whole)):
                ...
                num_instances_tmp = len(gt_kps[id])
                num_instances = num_instances_tmp
                keep_id = []
                for person in range(num_instances_tmp):
                    flag = False
                    kp = gt_kps[id][person]
                    for joint_id in range(num_joints):
                        ...
                        if  kp[joint_id_coco*3 +2] == 0 :
                            ...
                            continue
                        flag = True
                        masks[joint_id,kp[joint_id_coco*3+1],kp[joint_id_coco*3 ]] = 1
                    if not flag:
                        num_instances = num_instances -1
                    else :
                        keep_id.append(person)
                # no value box and joints in this image,
                # dump this image
                if num_instances == 0:
                    continue
                boxes = np.array(gt_boxes[id])[keep_id]
                ...
                example = _to_tfexample_raw(
                    imgId,img_raw, height, width, boxes_raw,masks_raw,num_instances
                )
...
def run(dataset_dir,dataset_split_name='train2014',npz_path = None):
    ...
    npz_path = '/home/hpc/ssd/lyj/Multi-tasks/annotation.npz'
    if not tf.gfile.Exists(record_dir):
        tf.gfile.MakeDirs(record_dir)

    if dataset_split_name in ['train2014','val2014']:
        _add_to_tfrecord(record_dir,dataset_dir,annotation_path,dataset_split_name,npz_path)

    print ('\nFinished write mpii dataset to tf_record')

if __name__ == '__main__':
    run('/home/hpc/ssd/lyj/Multi-tasks/data/coco')
