...
def solve(global_step):
    ...
    # learning reate
    lr = _configure_learning_rate(10000, global_step)
    ...
    # collect the differet multi-task losses
    rpn_box_losses = tf.get_collection("rpn_box_losses")
    rpn_cls_losses = tf.get_collection("rpn_cls_losses")
    refined_box_losses = tf.get_collection("refined_box_losses")
...
def train():
    ...
    ## data
    image,ih,iw,gt_boxes,gt_masks,gt_body_masks,num_masks,image_id = \
    ...
    enqueue_op = data_queue.enqueue((image,ih,iw,gt_boxes,gt_masks,gt_body_masks,num_masks,image_id))
    data_queue_runner = tf.train.QueueRunner(data_queue, [enqueue_op] * 4)
    tf.add_to_collection(tf.GraphKeys.QUEUE_RUNNERS, data_queue_runner)
    (image,ih,iw,gt_boxes,gt_masks,gt_body_masks,num_masks,image_id) =  data_queue.dequeue()
    ...
    for step in range(sess.run(global_step),FLAGS.max_iters):
        ...
        rpn_box_loss, rpn_cls_loss, refined_box_loss, refined_cls_loss, mask_loss,mask1,body_masks_loss, \
        gt_boxesnp, \
        ...
        if step % 1 == 0:
            print ( """iter %d: image-id:%07d, time:%.3f(sec), regular_loss: %.6f, """
                    """total-loss %.4f(%.4f, %.4f, %.6f, %.4f, %.8f, %.8f,%.4f), """
                    """instances: %d, """
                    """batch:(%d|%d, %d|%d, %d|%d)"""
                   % (step, img_id_str, duration_time, reg_lossnp,
                      tot_loss, rpn_box_loss, rpn_cls_loss, refined_box_loss, refined_cls_loss, mask_loss,mask1,body_masks_loss,
                      gt_boxesnp.shape[0],
                      rpn_batch_pos, rpn_batch, refine_batch_pos, refine_batch, body_mask_batch_pos, body_mask_batch))
