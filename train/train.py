...
def solve(global_step):
    ...
    # learning reate
    lr = _configure_learning_rate(24987, global_step)
...
def restore(sess):
    ...
    if FLAGS.restore_previous_if_exists:
        try:
            ...
            time.sleep(2)
            counter = int(next(re.finditer("(\d+)(?!.*\d)",checkpoint_path)).group(0))
            return counter
        except:
...
def train():
    ...
    ## restore
    checkpoint_counter = restore(sess)
    ...
    for step in range(checkpoint_counter,FLAGS.max_iters):
        ...
        if step % 1 == 0:
            ...
            # print ('final_label:', final_clsnp)
            # print (final_clsnp.shape)
            # print ('final_boxnp:', final_boxnp)
            # print (final_boxnp.shape)
            # print ('final_prob:', final_probnp)
            # print (final_probnp.shape)
            # raise
