...
class anchor_test(layer_test):
    ...
    def test(self):
        ...
        with tf.Session() as sess:
            all_anchors = gen_all_anchors(self.height / 4, self.width / 4, stride = 4, scales = 2**np.arange(1,2))
            ...
            print (self.labels)
            print ('bbox_targets.shape:',self.bbox_targets.shape)
            print ('bbox_targetsis :',self.bbox_targets)
            print ('bbox_inside_wight.shape:',self.bbox_inside_weights.shape)
            print (self.bbox_inside_weights)
            print ('gt_boxes',self.gt_boxes)
            # print (self.gt_boxes)
            ...
            print ('classes is :',classes.eval())
            print ('npboxes:',self.npboxes)
            npscores = scores.eval().reshape((-1, 1))
            print ('npscores:',npscores)
            ...
            print ('all_anchors:',all_anchors_np)
...
class roi_test(layer_test):
    ...
    def test(self):
        ...
        with tf.Session() as sess:
            ...
            self.rois = rois
            print ('rois:',rois)
            print ('self.gt_boxes:',self.gt_boxes)
            labels, bbox_targets, bbox_inside_weights = \
            ...
            print ('labelshape:',self.labels.shape)
            print ('label',self.labels)
            print ('bbox_targets shape:',self.bbox_targets.shape)
            print ('bbox_inside_weights shape:',self.bbox_inside_weights.shape)
            print ('bbox_inside_weights:',self.bbox_inside_weights)
...
class mask_test(layer_test):
    ...
    def test(self):
        ...
        with tf.Session() as sess:
            ...
            self.gt_masks = gt_masks
            print ('gt_mask shape: ',self.gt_masks.shape)
            # print ('gt_mask: ',self.gt_masks)

            labels, mask_targets, mask_inside_weights = \
                    mask_encoder(self.gt_masks, self.gt_boxes, rois, self.num_classes, 10, 10)
            self.labels = labels.eval()
            self.mask_targets = mask_targets.eval()
            self.mask_inside_weights = mask_inside_weights.eval()
            print ('mask_traget shape:',self.mask_targets.shape)
            print ('mask_inside_weights shape: ', self.mask_inside_weights.shape)
            # print (self.mask_targets)
            print (self.labels)
            for i in range(rois.shape[0]):
                print(i, 'label:', self.labels[i])
                print ('mask_targets:',self.mask_targets[i, :, :, int(self.labels[i])])
                print ('******************************')
                print ('mask_inside_weights: ',self.mask_inside_weights[i,:,:,:])
                print ('******************************')
class sample_test(layer_test):
    ...
    def test(self):
        with tf.Session() as sess:
            ...
            print('before sample:',np.hstack((boxes,scores)))
...
if __name__ == '__main__':
    ...
    # print ('Anchor Test')
    # print ('##############################')
    # an_test = anchor_test(0, 5, 24, 24)
    # an_test.test()
    # print ('*************************************************************************')
    # an_test = anchor_test(5, 5, 24, 24)
    # an_test.test()
    # print ('##############################')
    # print ('ROI Test')
    # print ('##############################')
    # r_test  = roi_test(2, 5, 24, 24)
    # r_test.test()
    # print ('*********************************************************************************')
    # r_test  = roi_test(0, 9, 24, 24)
    # r_test.test()
    print ('##############################')
    print ('Mask Test')
    print ('##############################')
