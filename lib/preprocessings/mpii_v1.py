...
def preprocess_image(image, gt_boxes, gt_masks, gt_body_masks,is_training=False):
...
def preprocess_for_training(image, gt_boxes, gt_masks,gt_body_masks):
    ...
    gt_body_masks = tf.image.resize_nearest_neighbor(gt_body_masks,[new_ih,new_iw],align_corners = False)
    ...
    gt_masks = tf.image.resize_nearest_neighbor(gt_masks,[tf.div(new_ih,8),tf.div(new_iw,8)],align_corners = False)
