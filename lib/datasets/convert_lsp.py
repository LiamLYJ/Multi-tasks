...
def _mask_encode(masks,viss,ih,iw,stride = None,num_joints = None,hp = False):
    if stride is None:
    ...
    if hp :
        ...
        for k in range(num_joints):
            if (viss[k] == 1):
                ...
                mask_targets[int(min_y):int(max_y),int(min_x):int(max_x),k] = 1
    else:
        for k in range(num_joints):
            j_y,j_x = 0,0
    ...
    for shard_id in range(num_shards):
        ...
        with tf.python_io.TFRecordWriter(record_filename) as tfrecord_writer:
            ...
            for image_id in range(start_ndx,end_ndx):
                ...
                gt_masks = _mask_encode(masks,viss,height,width)
...
if __name__ == '__main__':
    ...
    check = tmp_mat['joints'][:,:,4]
    img_path = '/home/hpc/ssd/lyj/FastMaskRCNN/data/lsp/images/im00005.jpg'
    ...
    #
    gt_masks = _mask_encode(masks,viss,ih,iw,30,14)
    ...
    # raise
    # cv2.imshow('gt_masks:',gt_masks[:,:,0])
